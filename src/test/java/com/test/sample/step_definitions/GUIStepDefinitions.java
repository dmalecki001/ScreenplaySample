...
import com.test.sample.selenium.webdriver_factory.WebDriverProvider;
import cucumber.api.java8.En;
import net.serenitybdd.screenplay.abilities.BrowseTheWeb;
import net.serenitybdd.screenplay.actions.Click;
import net.serenitybdd.screenplay.actions.Open;

import static com.test.sample.utils.TestDataManager.*;
...
public class GUIStepDefinitions extends BaseDefinitions implements En {
    ...
    public GUIStepDefinitions() {
        ...
        Given("(.*) can browse the web", (String actor) ->
                theActorCalled(actor).whoCan(BrowseTheWeb.with(new WebDriverProvider().newDriver())));

        When("(.*) attempts to provide data for new account registration", (String actor) -> {
            theActorCalled(actor).attemptsTo(Click.on(LoginPageTargets.getSignInBtn()),
        ...
        Then("(.*) should verify that the account was successfully created", (String actor) ->
                theActorCalled(actor).attemptsTo(Click.on(CreateAccountTargets.getRegisterButton()),
                        VerifyWebPageTitle.wasVisible(CreateAccountTargets.MY_ACCOUNT_PAGE_TITLE)));
        ...
        When("(.*) attempts to provide login data", (String actor) -> {
            theActorCalled(actor).whoCan(Authenticate
                    .withCredentials(getGuiEmail(),getGuiPassword()));
            theActorCalled(actor).attemptsTo(LogIntoTheAccount.withCredentials());
        });

        Then("(.*) should successfully log into the account", (String actor) ->
                theActorCalled(actor).attemptsTo(VerifyWebPageTitle
        ...
        When("(.*) attempts to add a new address", (String actor) ->
                theActorCalled(actor).attemptsTo(Click.on(MyAccountTargets.addNewAddressTab()),
